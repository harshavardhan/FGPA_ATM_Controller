    ...
    if (loopOpt->count > 0) {
        if (isCommCapable) {
            ...
            if (isRunning) {
                ...
                /*
                    Format for initialisation of Max Amount which can be withdrawn in a transaction and
                    cap on number of notes to be dispensed

                    MAX Amount in a transaction (32 bit) | 2k cap (8 bit) | 1k cap (8 bit) | 500 cap (8 bit) | 100 cap (8 bit)
                    (Before encryption)
                */
                uint32_t length = 1;
                uint32_t befEncSenInit[2];
                for (int i = 0; i < 2; i++) befEncSenInit[i] = 0;
                befEncSenInit[1] = maxAmountCanBeDispensed;
                for (uint32_t i = 0; i <= 31; i += 8) {
                    if (i == 0) befEncSenInit[0] += ((1 << i) * ((uint32_t) max100Limit));
                    else if (i == 8) befEncSenInit[0] += ((1 << i) * ((uint32_t) max500Limit));
                    else if (i == 16) befEncSenInit[0] += ((1 << i) * ((uint32_t) max1000Limit));
                    else befEncSenInit[0] += ((1 << i) * ((uint32_t) max2000Limit));
                }
                encrypt64(befEncSenInit);

                for (uint8_t i = 18; i <= 21; i++) {
                    uint8_t tempSto = 0;
                    for (uint8_t j = 0; j <= 7; j++) {
                        uint8_t temp = j + (i - 18) * 8;
                        if ((befEncSenInit[0] & (1 << temp)) != 0) {
                            tempSto += (1 << j);
                        }
                    }
                    flSleep(1000);
                ...
                for (uint8_t i = 22; i <= 25; i++) {
                    uint8_t tempSto = 0;
                    for (uint8_t j = 0; j <= 7; j++) {
                        uint8_t temp = j + (i - 22) * 8;
                        if ((befEncSenInit[1] & (1 << temp)) != 0) {
                            tempSto += (1 << j);
                        }
                    }
                    flSleep(1000);
